...
import {
  oracleMasterSourceV1CodeCell,
  oracleClientSourceV1CodeCell,
} from "./OracleV1.source";
...
export function buildOracleMetadataCell(data: {
  ...
  Object.entries(data).forEach(([k, v]: [string, string | undefined]) => {
    ...
    let bufferToStore = Buffer.from(
      v,
      oracleOnChainMetadataSpec[k as OracleMetaDataKeys]
    );
...
export function oracleMasterInitData(metadata: {
  [s in OracleMetaDataKeys]?: string;
}): Cell {
  return beginCell()
